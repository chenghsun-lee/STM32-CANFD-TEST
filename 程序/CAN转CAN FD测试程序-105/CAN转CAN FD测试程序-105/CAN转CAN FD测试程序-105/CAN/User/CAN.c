...
CanRxMsg CAN1_RxMessage[CAN_BUF_NUM];
CanRxMsg CAN2_RxMessage;

uint8_t CAN_Send_Num = 0;
uint8_t CAN_Recv_Num = 0;
...
uint8_t CAN1_wrMsg (CanTxMsg msg) 
{	
	uint32_t can_times = 0;
	uint8_t can_mailbox=0;
	
	uint8_t send_ok = 0;

	can_mailbox = CAN_Transmit(CAN1, &msg);
	
	while(CAN_TxStatus_Ok !=CAN_TransmitStatus(CAN1,can_mailbox))
	{
		can_times++;
		if(can_times >=100000)
		{
...
uint8_t CAN2_wrMsg (CanTxMsg msg) 
{	
	uint32_t can_times = 0;
	uint8_t can_mailbox=0;
...
#endif
{
  ...
  CAN_Receive(CAN1,CAN_FIFO0,&CAN1_RxMessage[CAN_Recv_Num]);  /* 此函数包含释放提出报文了的,在非必要时,不需要自己释放 */
	CAN_Recv_Num++;
	if(CAN_Recv_Num >= CAN_BUF_NUM)
		CAN_Recv_Num = 0;
