 ...
 *      RL-ARM - USB
 *----------------------------------------------------------------------------
 *      Name:    usbd_STM32F10x_cl.c
 *      Purpose: Hardware Layer module for ST STM32F107CV
 *      Rev.:    V4.70
 *----------------------------------------------------------------------------
...
uint32_t OutMaxPacketSize[4] = {USBD_MAX_PACKET0,0,0,0};
...
void USBD_Init (void) {
  ...
  OTG_FS->GUSBCFG  = (OTG_FS->GUSBCFG & ~(15 << 10)) |(9 << 10);  /*turnaround*/

  OTG_FS->DCFG    |=  3;                /* full speed phy                     */
  OTG_FS->PCGCCTL  =  0;                /* Restart PHY clock                  */

  OTG_FS->GINTMSK  = (1   << 11) |      /* suspend int unmask                 */
                     (1   << 12) |      /* reset int unmask                   */
                     (1   << 13) |      /* enumeration done int unmask        */
                     (1   << 4 ) |      /* recive fifo non-empty int  unmask  */
...
void USBD_Reset (void) {
  ...
  OTG_FS->DAINTMSK = 1 | (1 << 16);     /* unmask IN&OUT EP0 interruts        */

  OTG_FS->DOEPMSK  =  (1 << 3) |        /* setup phase done                   */
                      (1 << 1) |        /* endpoint disabled                  */
                       1;               /* transfer complete                  */
  OTG_FS->DIEPMSK  =  (1 << 1) |        /* endpoint disabled                  */
                       1;               /* transfer completed                 */

  OTG_FS->GRXFSIZ   =   RX_FIFO_SIZE/4;
  OTG_FS->GNPTXFSIZ =  (RX_FIFO_SIZE/4) | ((TX0_FIFO_SIZE/4) << 16);

  OTG_FS->DIEPTXF1 =  ((RX_FIFO_SIZE + TX0_FIFO_SIZE)/4) |
                      ((TX1_FIFO_SIZE/4) << 16);

  OTG_FS->DIEPTXF2 =  ((RX_FIFO_SIZE + TX0_FIFO_SIZE + TX1_FIFO_SIZE)/4) |
                      ((TX2_FIFO_SIZE/4) << 16);

  OTG_FS->DIEPTXF3 =  ((RX_FIFO_SIZE + TX0_FIFO_SIZE+ TX1_FIFO_SIZE +TX2_FIFO_SIZE)/4) |
                      ((TX3_FIFO_SIZE/4) << 16);

  OTG_FS->DOEPTSIZ0  =  (1 << 29) |         /* setup count = 1                */
                        (1 << 19) |         /* packet count                   */
                         USBD_MAX_PACKET0;
}
...
static void USBD_FlushInEpFifo (uint32_t EPNum) {
  ...
  OTG_FS->GRSTCTL = (OTG_FS->GRSTCTL & ~(0x1F << 6)) | /* flush EP fifo       */
                    (EPNum << 6)| (1 << 5);
  wcnt = 10;
  while (OTG_FS->GRSTCTL & (1 << 5))    /* wait until fifo is flushed         */
    if ((wcnt--) == 0) break;
}
...
void USBD_ConfigEP (USB_ENDPOINT_DESCRIPTOR *pEPD) {
  ...
  if (pEPD->bEndpointAddress & USB_ENDPOINT_DIRECTION_MASK) {
    OTG_FS->DAINTMSK |= (1    << num);  /* unmask IN EP int                   */
    ...
    OTG_FS->DAINTMSK |= (1 << (num + 16));/* unmask OUT EP int                */
...
void USBD_EnableEP (uint32_t EPNum) {
  if (EPNum & 0x80) {
    ...
    if (EP_IN_TYPE(EPNum) == USB_ENDPOINT_TYPE_ISOCHRONOUS) {
      OTG_FS->GINTMSK |= (1 << 20);     /* enable IISOIXFR                    */
    }
  } else {
    if (EP_OUT_TYPE(EPNum) == USB_ENDPOINT_TYPE_ISOCHRONOUS) {
      OTG_FS->GINTMSK |= (1 << 15);     /* enable end of periodic frame       */
...
void USBD_DisableEP (uint32_t EPNum) {
  ...
  if (EPNum & 0x80) {
    ...
    if (IsoEpEnCnt == 1) {              /* if all iso endpoints disabled      */
      OTG_FS->GINTMSK &= ~(1 << 20);    /* disable IISOIXFR                   */
    }
    if (DIEPCTL(EPNum) &   (1UL << 31))
      DIEPCTL(EPNum)   |=  (1   << 30); /* disable EP                         */
  ...
  /* Disable OUT Endpoint                                                     */
    ...
    if (IsoEpEnCnt == 1)                /* if all iso endpoints disabled      */
      OTG_FS->GINTMSK &= ~(1 << 15);    /* disable EOPF                       */
    
    OTG_FS->DCTL |= (1 << 9);           /* set global out nak                 */

    wcnt = 1000;
    while (!(OTG_FS->GINTSTS & (1 << 7))) /* wait until global NAK            */
      if ((wcnt--) == 0) break;
    ...
    while(!(DOEPINT(EPNum) & (1 << 1)))   /* wait until EP disabled           */
      if ((wcnt--) == 0) break;
    OTG_FS->DCTL |= (1 << 10);            /* clear global nak                 */
  }
}
...
uint32_t USBD_ReadEP (uint32_t EPNum, uint8_t *pData) {
  ...
  sz = ( OTG_FS->GRXSTSP >> 4) & 0x7FF; /* get available data size            */
  if (isoEpFlag) {
  ...
  while ((OTG_FS->GINTSTS & (1 << 4)) == 0);  
  OTG_FS->GRXSTSP;                      /* pop register                       */
  OTG_FS->GINTMSK |= (1 << 4);          /* unmask RxFIFO non-empty interrupt  */
  ...
  if (!SyncWriteEP && InPacketDataPtr[EPNum]) {
    if ((!(InPacketDataReady & (1 << EPNum))) || isoEpFlag) {
      ...
      if (!isoEpFlag) {
        ...
        OTG_FS->DIEPMSK   |= (1 <<  6); /* INEPNEM = 1, IN EP NAK efective msk*/
      }
...
/* OUT Packet                                                                 */
  if (istr & (1 << 19)) {
    msk = (((OTG_FS->DAINT & OTG_FS->DAINTMSK) >> 16) & 0xFFFF);
    i   = 0;
    while (msk) {
      num = 0;
...
/* IN Packet                                                                  */
  if (istr & (1 << 18)) {
    msk = (OTG_FS->DAINT & OTG_FS->DAINTMSK & 0xFFFF);
    i   = 0;
    while (msk) {
      num = 0;
      for (; i < (USBD_EP_NUM+1); i++) {
        ...
        if (EP_IN_TYPE(num) != USB_ENDPOINT_TYPE_ISOCHRONOUS) {
          if (InPacketDataPtr[num] && (InPacketDataReady & (1 << num))) {
            ...
            if (!InPacketDataReady)         /* No more pending IN transfers   */
              OTG_FS->DIEPMSK &= ~(1 << 6); /* Disable IN NAK interrupts      */
            continue;
          } else 
              DIEPCTL(num) |= (1 << 26);
            DIEPINT(num)    = (1 <<  6);
